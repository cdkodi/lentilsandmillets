-- =============================================================================
-- CLEAN CMS SCHEMA CREATION
-- Migration: 002_create_clean_cms_schema.sql
-- Creates new CMS tables with different names to avoid Payload conflicts
-- =============================================================================

-- Drop any existing CMS tables (if they exist)
DROP TABLE IF EXISTS cms_articles CASCADE;
DROP TABLE IF EXISTS cms_recipes CASCADE;
...
CREATE INDEX idx_cms_articles_card_position ON cms_articles(card_position);
...
CREATE INDEX idx_cms_articles_display_pages ON cms_articles USING GIN(display_pages);
CREATE INDEX idx_cms_articles_tags ON cms_articles USING GIN(tags);

-- CMS Recipes indexes
CREATE INDEX idx_cms_recipes_card_position ON cms_recipes(card_position);
CREATE INDEX idx_cms_recipes_category ON cms_recipes(category);
CREATE INDEX idx_cms_recipes_status ON cms_recipes(status);
CREATE INDEX idx_cms_recipes_is_featured ON cms_recipes(is_featured);
CREATE INDEX idx_cms_recipes_meal_type ON cms_recipes(meal_type);
CREATE INDEX idx_cms_recipes_dietary_tags ON cms_recipes USING GIN(dietary_tags);
CREATE INDEX idx_cms_recipes_display_pages ON cms_recipes USING GIN(display_pages);
CREATE INDEX idx_cms_recipes_published_at ON cms_recipes(published_at);

-- =============================================================================
-- CARD POSITION VALIDATION CONSTRAINTS
-- =============================================================================

-- Prevent invalid card position assignments for cms_articles
ALTER TABLE cms_articles ADD CONSTRAINT valid_cms_article_card_assignment CHECK (
...
-- Prevent invalid card position assignments for cms_recipes
ALTER TABLE cms_recipes ADD CONSTRAINT valid_cms_recipe_card_assignment CHECK (
  card_position IS NULL OR
  (
...
-- =============================================================================
-- UNIQUE CONSTRAINTS FOR CARD POSITIONS
-- =============================================================================

-- Prevent more than 1 content per card position
CREATE UNIQUE INDEX idx_unique_cms_card_position_articles 
ON cms_articles (card_position) 
WHERE card_position IS NOT NULL AND status = 'published';

CREATE UNIQUE INDEX idx_unique_cms_card_position_recipes 
ON cms_recipes (card_position) 
WHERE card_position IS NOT NULL AND status = 'published';
...
CREATE OR REPLACE FUNCTION calculate_cms_total_time()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_calculate_cms_total_time
  BEFORE INSERT OR UPDATE ON cms_recipes
  FOR EACH ROW
  EXECUTE FUNCTION calculate_cms_total_time();
...
CREATE TRIGGER trigger_cms_articles_updated_at
  BEFORE UPDATE ON cms_articles
  FOR EACH ROW
  EXECUTE FUNCTION update_cms_updated_at_column();

CREATE TRIGGER trigger_cms_recipes_updated_at
  BEFORE UPDATE ON cms_recipes
  FOR EACH ROW
  EXECUTE FUNCTION update_cms_updated_at_column();
...
-- Sample green lentils article
INSERT INTO cms_articles (
  title, slug, content, excerpt, hero_image_url, author, category,
  card_position, display_pages, factoid_data, meta_title, tags, status, published_at
) VALUES (
  'Green Lentils: The Fiber Champion',
  'green-lentils-fiber-champion',
  'Green lentils are nutritional powerhouses known for their exceptional fiber content and hearty texture. Unlike red lentils, green lentils hold their shape when cooked, making them perfect for salads, stews, and grain bowls.',
  'Learn why green lentils are considered the fiber champion among legumes and how to cook them perfectly.',
  'https://images.unsplash.com/photo-1567306301408-e6d4607c2b51?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80',
  'Dr. Sarah Johnson',
  'lentils',
  'H5',
  ARRAY['home', 'lentils'],
  ...
  'Green Lentils: High Fiber Nutrition Guide | Lentils & Millets',
  ARRAY['green lentils', 'fiber', 'digestive health', 'nutrition'],
  'published',
  NOW()
);

-- Sample millet article
INSERT INTO cms_articles (
  title, slug, content, excerpt, hero_image_url, author, category,
  card_position, display_pages, factoid_data, meta_title, tags, status, published_at
) VALUES (
  ...
  'pearl-millet-climate-resilient-champion',
  'Pearl millet is one of the most drought-resistant crops on Earth, capable of growing in areas with minimal rainfall and extreme temperatures. This ancient grain is not only sustainable but also incredibly nutritious.',
  'Discover how pearl millet is helping fight climate change while providing superior nutrition.',
  'https://images.unsplash.com/photo-1603833797131-3d9e8cff5058?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80',
  'Dr. Climate Expert',
  'millets',
  'H12',
  ARRAY['home', 'millets'],
  ...
  'Pearl Millet: Climate-Resilient Ancient Grain | Lentils & Millets',
  ARRAY['pearl millet', 'climate change', 'sustainability', 'drought resistant'],
  'published',
  NOW()
);
...
WHERE table_schema = 'public' 
  AND table_name IN ('cms_articles', 'cms_recipes')
ORDER BY table_name;

-- Show sample data counts
SELECT 'Sample Data:' as status;
SELECT 
  'cms_articles' as table_name,
...
SELECT 
  'cms_recipes' as table_name,
  count(*) as record_count  
FROM cms_recipes
WHERE status = 'published';

-- Show card assignments
SELECT 'Card Assignments:' as status;
SELECT 
  card_position,
